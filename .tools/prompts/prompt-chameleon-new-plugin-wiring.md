...
**Companion:** Use **`prompt-wordpress-plugin-starter.md`** first (or in parallel) for `/plugin` layout, versioning, Git, headers, and Chameleon product rules. **This prompt** adds only what is required to wire the plugin into **`live-plugins`**, **`publish-live-plugins.ps1`**, and **optional SFTP** — not duplicate the starter.

**Authoritative docs:** `.docs/REPO-AND-PUBLISH-LAYOUT.md`, `live-plugins/README.md`, `.cursor/rules/live-plugins-latest.mdc`, `.cursor/rules/plugin-zip-on-change.mdc`, `.cursor/rules/plugin-install-zip-format.mdc`.
...
| 7 | **Update server** | If this plugin should receive updates via the Chameleon library: exact **`Update URI:`** line (see `live-plugins/README.md`) and confirm **Chameleon Plugin Updates** connector behaviour. |
...
This triggers **`publish-live-plugins.ps1 -RepoRoot <plugin>`**, which copies the newest matching zip(s) into **`live-plugins/`**, regenerates **`index.json`**, and runs **SFTP** when `CHAMELEON_LIVE_PLUGINS_SFTP_*` (or legacy FTP-named aliases) are set — see `live-plugins/README.md`.

### 2.5 `.gitignore`

- Ensure plugin repo **`.gitignore`** excludes **`dist/**/*.zip`** (and any other generated artifacts per starter). Zips must not be committed in the plugin repo; **`live-plugins/*.zip`** are ignored at the monorepo per existing policy.
...
- If SFTP is not configured, local **`live-plugins/`** and **`index.json`** update still prove success.
...
If this prompt introduces a **new long-lived** convention not already captured, add or update **`.cursor/rules/*.mdc`** per `.cursor/rules/prompts-to-rules.mdc`. For standard Chameleon plugins, existing **live-plugins** and **plugin-zip** rules usually suffice.
